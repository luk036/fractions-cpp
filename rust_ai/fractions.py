...
"""ExtFraction, infinite-precision, rational numbers."""
...
def _hash_algorithm(numerator, denominator):
    ...
    # To make sure that the hash of a ExtFraction agrees with the hash
...
class ExtFraction(numbers.Rational):
    """This class implements rational numbers.

    In the two-argument form of the constructor, ExtFraction(8, 6) will
    produce a rational number equivalent to 4/3. Both arguments must
    be Rational. The numerator defaults to 0 and the denominator
    defaults to 1 so that ExtFraction(3) == 3 and ExtFraction() == 0.
    ...
    def __new__(cls, numerator=0, denominator=None):
        ...
        >>> ExtFraction(10, -8)
        ExtFraction(-5, 4)
        >>> ExtFraction(ExtFraction(1, 7), 5)
        ExtFraction(1, 35)
        >>> ExtFraction(ExtFraction(1, 7), ExtFraction(2, 3))
        ExtFraction(3, 14)
        >>> ExtFraction('314')
        ExtFraction(314, 1)
        >>> ExtFraction('-35/4')
        ExtFraction(-35, 4)
        >>> ExtFraction('3.1415') # conversion from numeric string
        ExtFraction(6283, 2000)
        >>> ExtFraction('-47e-2') # string may include a decimal exponent
        ExtFraction(-47, 100)
        >>> ExtFraction(1.47)  # direct construction from float (exact conversion)
        ExtFraction(6620291452234629, 4503599627370496)
        >>> ExtFraction(2.25)
        ExtFraction(9, 4)
        >>> ExtFraction(Decimal('1.47'))
        ExtFraction(147, 100)

        """
        self = super(ExtFraction, cls).__new__(cls)
        ...
        if denominator is None:
            if type(numerator) is int:
                ...
                if m is None:
                    raise ValueError('Invalid literal for ExtFraction: %r' %
                                     numerator)
        ...
        if denominator == 0:
            raise ZeroDivisionError('ExtFraction(%s, 0)' % numerator)
    ...
    def from_float(cls, f):
        ...
        Beware that ExtFraction.from_float(0.3) != ExtFraction(3, 10).
    ...
    def _from_coprime_ints(cls, numerator, denominator, /):
        ...
        should be positive.
        """
        obj = super(ExtFraction, cls).__new__(cls)
        obj._numerator = numerator
        obj._denominator = denominator
        return obj

    def is_integer(self):
        """Return True if the ExtFraction is an integer."""
        return self._denominator == 1

    def as_integer_ratio(self):
        """Return a pair of integers, whose ratio is equal to the original ExtFraction.
    ...
    def limit_denominator(self, max_denominator=1000000):
        """Closest ExtFraction to self with denominator at most max_denominator.

        >>> ExtFraction('3.141592653589793').limit_denominator(10)
        ExtFraction(22, 7)
        >>> ExtFraction('3.141592653589793').limit_denominator(100)
        ExtFraction(311, 99)
        >>> ExtFraction(4321, 8765).limit_denominator(10000)
        ExtFraction(4321, 8765)
        ...
        if 2*d*(q0+k*q1) <= self._denominator:
            return ExtFraction._from_coprime_ints(p1, q1)
        else:
            return ExtFraction._from_coprime_ints(p0+k*p1, q0+k*q1)
    ...
    def _operator_fallbacks(monomorphic_operator, fallback_operator):
        ...
        ExtFraction, that means that we define __add__ and __radd__ as:

            def __add__(self, other):
                # Both types have numerators/denominator attributes,
                # so do the operation directly
                if isinstance(other, (int, ExtFraction)):
                    return ExtFraction(self.numerator * other.denominator +
        ...
        ExtFraction. I'll refer to all of the above code that doesn't
        refer to ExtFraction, float, or complex as "boilerplate". 'r'
        will be an instance of ExtFraction, which is a subtype of
        Rational (r : ExtFraction <: Rational), and b : B <:
        Complex. The first three involve 'r + b':

            1. If B <: ExtFraction, int, float, or complex, we handle
               that specially, and all is well.
            2. If ExtFraction falls back to the boilerplate code, and it
            ...
            3. If B <: ExtFraction, Python tries B.__radd__ before
               ExtFraction.__add__. This is ok, because it was
               implemented with knowledge of ExtFraction, so it can
               handle those instances before delegating to Real or
               Complex.

        The next two situations describe 'b + r'. We assume that b
        didn't know about ExtFraction in its implementation, and that it
        ...
        """
        def forward(a, b):
            if isinstance(b, ExtFraction):
                return monomorphic_operator(a, b)
            elif isinstance(b, int):
                return monomorphic_operator(a, ExtFraction(b))
        ...
        def reverse(b, a):
            if isinstance(a, numbers.Rational):
                # Includes ints.
                return monomorphic_operator(ExtFraction(a), b)
    ...
    def _add(a, b):
        ...
        g = math.gcd(da, db)
        if g == 1:
            return ExtFraction._from_coprime_ints(na * db + da * nb, da * db)
        s = da // g
        t = na * (db // g) + nb * s
        g2 = math.gcd(t, g)
        if g2 == 1:
            return ExtFraction._from_coprime_ints(t, s * db)
        return ExtFraction._from_coprime_ints(t // g2, s * (db // g2))
    ...
    def _mul(a, b):
        ...
        if g2 > 1:
            nb //= g2
            da //= g2
        return ExtFraction._from_coprime_ints(na * nb, db * da)
    ...
    def _div(a, b):
        ...
        nb, db = b._numerator, b._denominator
        if nb == 0:
            raise ZeroDivisionError('ExtFraction(%s, 0)' % db)
        na, da = a._numerator, a._denominator
        g1 = math.gcd(na, nb)
        if g1 > 1:
            na //= g1
        ...
        if d < 0:
            n, d = -n, -d
        return ExtFraction._from_coprime_ints(n, d)
    ...
    def _divmod(a, b):
        ...
        return div, ExtFraction(n_mod, da * db)
    ...
    def _mod(a, b):
        """a % b"""
        da, db = a.denominator, b.denominator
        return ExtFraction((a.numerator * db) % (b.numerator * da), da * db)
    ...
    def __pow__(a, b):
        ...
        if isinstance(b, numbers.Rational):
            if b.denominator == 1:
                power = b.numerator
                if power >= 0:
                    return ExtFraction._from_coprime_ints(a._numerator ** power,
                                                       a._denominator ** power)
                elif a._numerator > 0:
                    return ExtFraction._from_coprime_ints(a._denominator ** -power,
                                                       a._numerator ** -power)
                elif a._numerator == 0:
                    raise ZeroDivisionError('ExtFraction(%s, 0)' %
                                            a._denominator ** -power)
                else:
                    return ExtFraction._from_coprime_ints((-a._denominator) ** -power,
                                                       (-a._numerator) ** -power)
    ...
    def __rpow__(b, a):
        ...
        if isinstance(a, numbers.Rational):
            return ExtFraction(a.numerator, a.denominator) ** b
    ...
    def __pos__(a):
        """+a: Coerces a subclass instance to ExtFraction"""
        return ExtFraction._from_coprime_ints(a._numerator, a._denominator)
    ...
    def __abs__(a):
        """abs(a)"""
        return ExtFraction._from_coprime_ints(abs(a._numerator), a._denominator)
    ...
    def __round__(self, ndigits=None):
        ...
        # these operations will always be ExtFraction and therefore have
        # round().
        if ndigits > 0:
            return ExtFraction(round(self * shift), shift)
        else:
            return ExtFraction(round(self / shift) * shift)
