...
TEST_CASE("abs function edge cases") {
    SUBCASE("With signed types") {
        ...
        CHECK_EQ(abs(std::numeric_limits<int>::max()), std::numeric_limits<int>::max());
    }
    ...
    SUBCASE("With short type") {
        CHECK_EQ(abs(static_cast<short>(0)), 0);
        CHECK_EQ(abs(static_cast<short>(1)), 1);
        CHECK_EQ(abs(static_cast<short>(-1)), 1);
    }
}
...
TEST_CASE("GCD edge cases") {
    ...
    SUBCASE("With large numbers") {
        CHECK_EQ(gcd(1000000, 2000000), 1000000);
        CHECK_EQ(gcd(123456, 789012), 12);
    }

    SUBCASE("With prime numbers") {
        CHECK_EQ(gcd(17, 19), 1);
        CHECK_EQ(gcd(97, 89), 1);
    }
}
...
TEST_CASE("LCM edge cases") {
    ...
    SUBCASE("With coprime numbers") {
        CHECK_EQ(lcm(7, 11), 77);
        CHECK_EQ(lcm(13, 17), 221);
    }

    SUBCASE("With large numbers") {
        CHECK_EQ(lcm(12, 18), 36);
        CHECK_EQ(lcm(15, 25), 75);
    }
...
TEST_CASE("Cross product edge cases") {
    ...
    SUBCASE("With negative fractions") {
        ...
        CHECK_EQ(neg1.cross(neg2), -1);
        CHECK_EQ(neg2.cross(neg1), 1);
    }

    SUBCASE("With zero") {
        ExtFraction<int> zero(0, 1);
        CHECK_EQ(f1.cross(zero), 1);
        CHECK_EQ(zero.cross(f1), -1);
        CHECK_EQ(zero.cross(zero), 0);
    }

    SUBCASE("With unity") {
        ExtFraction<int> one(1, 1);
        CHECK_EQ(f1.cross(one), -1);
        CHECK_EQ(one.cross(f1), 1);
    }

    SUBCASE("Anti-symmetry property") {
        CHECK_EQ(f1.cross(f2), -f2.cross(f1));
        CHECK_EQ(f1.cross(f4), -f4.cross(f1));
    }
}
...
TEST_CASE("Stream operator with different scenarios") {
    ...
    SUBCASE("Large numbers") {
        ...
        CHECK_EQ(ss.str(), "(823/4526)");  // Normalized
    }
...
TEST_CASE("Mixed arithmetic operations chains") {
    SUBCASE("Long chain of additions") {
        ...
        // 0 + 1/1 + 1/2 + 1/3 + ... + 1/10
        CHECK_EQ(f.numer(), 7381);
        CHECK_EQ(f.denom(), 2520);
    }
    ...
    SUBCASE("Long chain of multiplications") {
        ...
        CHECK_EQ(f, ExtFraction<int>(5, 1));
    }
...
TEST_CASE("Special value arithmetic") {
    ...
    SUBCASE("Normal value + infinity") {
        ...
        CHECK_EQ(normal + inf, inf);
        CHECK_EQ(inf + normal, inf);
    }
...
TEST_CASE("Reciprocal edge cases") {
    ...
    SUBCASE("Reciprocal of zero") {
        ExtFraction<int> zero(0, 1);
        zero.reciprocal();
        CHECK_EQ(zero.numer(), 1);
        CHECK_EQ(zero.denom(), 0);  // Infinity
    }

    SUBCASE("Reciprocal of infinity") {
        ExtFraction<int> inf(1, 0);
        inf.reciprocal();
        CHECK_EQ(inf.numer(), 0);
        CHECK_EQ(inf.denom(), 1);  // Zero
    }

    SUBCASE("Reciprocal of negative infinity") {
        ExtFraction<int> neg_inf(-1, 0);
        neg_inf.reciprocal();
        CHECK_EQ(neg_inf.numer(), 0);
        CHECK_EQ(neg_inf.denom(), 1);  // Zero (normalized to 0/1)
    }
}
...
TEST_CASE("Increment/decrement with special values") {
    SUBCASE("Increment zero") {
        ExtFraction<int> zero(0, 1);
        ++zero;
        CHECK_EQ(zero, ExtFraction<int>(1, 1));
    }

    SUBCASE("Decrement unity") {
        ExtFraction<int> one(1, 1);
        --one;
        CHECK_EQ(one, ExtFraction<int>(0, 1));
    }
...
TEST_CASE("Arithmetic with large denominators") {
    ...
    SUBCASE("Multiplication with large denominators") {
        ...
        auto result = f1 * f2;
        CHECK_EQ(result, ExtFraction<int>(1, 1000000));
    }
    ...
    SUBCASE("Division with large denominators") {
        ...
        ExtFraction<int> f2(1, 1000);
        auto result = f1 / f2;
        CHECK_EQ(result, ExtFraction<int>(1000000, 1));
    }
}
